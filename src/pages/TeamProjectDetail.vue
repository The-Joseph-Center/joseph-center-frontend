...
onMounted(async () => {
  ...
  try {
    ...
    const query = encodeURIComponent(`*[_type == "teamProject" && !(_id in path("drafts.**")) && slug.current == "${slug}"][0]{ title, "slug": slug.current, category, description, image, imageAlt, logo, body }`);
    ...
    console.error('Failed to fetch team project:', e);
...
<template>
  <main class="page page--team-project-detail">
